...
def create_trainee(request):
    ...
    if request.method == 'POST':
        ...
        trainee.birth_date = request.POST.get('birth_date')  # Fixed typo
        trainee.address = request.POST.get('address')
        trainee.phone = request.POST.get('phone')
        trainee.email = request.POST.get('input_email')  # Ensure this matches the form

        # Handle Track lookup with error handling
        try:
            trainee.trackobj = Track.objects.get(id=request.POST.get('trackobj'))
        except Track.DoesNotExist:
            context['error'] = "The selected track does not exist."
